...
def AHA_global_route_adjustment(start_pos, target_pos, obstacles, params, pop_size=100, iters=50, n_waypoints=20, base_offsets=None):
    """
    Use AHA to optimize the N intermediate waypoints between start and target.
    """
    if not obstacles:
    ...
    def space_bound(vector):
        bounded = np.asarray(vector, dtype=float).copy()
        out_of_bounds = (bounded > upper_bound) | (bounded < lower_bound)
        if np.any(out_of_bounds):
            bounded[out_of_bounds] = (
                np.random.rand(np.count_nonzero(out_of_bounds))
                * (upper_bound[out_of_bounds] - lower_bound[out_of_bounds])
                + lower_bound[out_of_bounds]
            )
        return bounded

    npop = max(2, int(pop_size))
    max_it = max(0, int(iters))

    X = lower_bound + np.random.rand(npop, dim) * (upper_bound - lower_bound)
    fitness = np.array([evaluate_vector(individual) for individual in X], dtype=float)

    best_index = int(np.argmin(fitness))
    best_f = float(fitness[best_index])
    best_x = X[best_index].copy()
    his_best_fit = np.zeros(max_it, dtype=float)

    visit_table = np.zeros((npop, npop), dtype=float)
    diag_ind = np.diag_indices(npop)
    visit_table[diag_ind] = np.nan

    for gen in range(max_it):
        ...
        if getattr(params, 'print_ga_best_each_iter', False):
            ...
            if gen % interval == 0:
                _, best_dbg = calculate_path_fitness(decode(X[current_best_index]), start_pos, target_pos, obstacles, params, base_offsets)
                print(
                    f"[AHA-BEST] iter={gen:03d} J={best_dbg['J']:.6f} "
                    f"L={best_dbg['L']:.2f} S={best_dbg['S']:.2f} R={best_dbg['R']:.2f} "
                    f"W={best_dbg['W']:.4f} F={best_dbg['F']:.2f}"
                )

        if params.debug_fitness and (gen % 5 == 0):
            _, dbg = calculate_path_fitness(decode(X[current_best_index]), start_pos, target_pos, obstacles, params, base_offsets)
            print(
                f"[AHA] gen={gen:02d} J={dbg['J']:.4f} "
        ...
        for i in range(npop):
            direct_vector = np.zeros(dim, dtype=float)
            r = np.random.rand()

            if r < 1.0 / 3.0:
                rand_dims = np.random.permutation(dim)
                max_diag_dims = dim - 2 if dim >= 3 else max(dim - 1, 1)
                rand_num = int(np.ceil(np.random.rand() * max_diag_dims))
                rand_num = max(1, min(rand_num, dim))
                direct_vector[rand_dims[:rand_num]] = 1.0
            elif r > 2.0 / 3.0:
                direct_vector[:] = 1.0
            else:
                direct_vector[np.random.randint(0, dim)] = 1.0

            if np.random.rand() < 0.5:
                max_unvisited_time = np.max(visit_table[i, :])
                target_candidates = np.where(visit_table[i, :] == max_unvisited_time)[0]
                if target_candidates.size > 1:
                    target_food_index = int(target_candidates[np.argmin(fitness[target_candidates])])
                else:
                    target_food_index = int(target_candidates[0])

                new_pos = X[target_food_index, :] + np.random.randn() * direct_vector * (X[i, :] - X[target_food_index, :])
                new_pos = space_bound(new_pos)
                new_fit = evaluate_vector(new_pos)

                if new_fit < fitness[i]:
                    fitness[i] = new_fit
                    X[i, :] = new_pos
                    visit_table[i, :] += 1
                    visit_table[i, target_food_index] = 0
                ...
                new_pos = X[i, :] + np.random.randn() * direct_vector * X[i, :]
                new_pos = space_bound(new_pos)
                new_fit = evaluate_vector(new_pos)
        ...
        if np.mod(gen, 2 * npop) == 0:
            visit_table[diag_ind] = float('-inf')
            migration_index = int(np.argmax(fitness))
            X[migration_index, :] = lower_bound + np.random.rand(dim) * (upper_bound - lower_bound)
            visit_table[migration_index, :] += 1
            visit_table[:, migration_index] = np.max(visit_table, axis=1) + 1
            visit_table[migration_index, migration_index] = float('-inf')
            fitness[migration_index] = evaluate_vector(X[migration_index, :])
            visit_table[diag_ind] = np.nan

        current_best_index = int(np.argmin(fitness))
        if fitness[current_best_index] < best_f:
            best_f = float(fitness[current_best_index])
            best_x = X[current_best_index].copy()
        his_best_fit[gen] = best_f

    best_path = decode(best_x)
    best_j, best_details = calculate_path_fitness(best_path, start_pos, target_pos, obstacles, params, base_offsets)
    if getattr(params, 'print_ga_best_each_iter', False) and best_details is not None:
        print(
            f"[AHA-FINAL-BEST] J={best_j:.6f} "
...
def run_point_to_point_formation():
    ...
    print("正在执行全局 AHA 路径规划...")
    global_waypoints = AHA_global_route_adjustment(
        start_pos,
        target_pos,
        obstacles,
        params,
    ...
    csv_filename = 'uav_states_aha_apf.csv'
    print(f"开始仿真并记录数据到 {csv_filename} (混合架构：Global AHA + Local APF)...")
    ...
    gif_name = 'uav_formation_p2p_AHA_apf_static.gif'
