...
def ga_global_route_adjustment(start_pos, target_pos, obstacles, params, pop_size=100, iters=50, n_waypoints=20, base_offsets=None):
    """
    基于遗传算法 (GA) 的全局路径规划：输出起始点到目标点之间的 N 个空间航点
    """
    if not obstacles:
    ...
    for _ in range(pop_size):
        waypoints = base_line + np.random.uniform(lower_delta, upper_delta)
        waypoints = np.clip(waypoints, waypoint_lower, waypoint_upper)
        fitness = evaluate(waypoints)
        population.append((fitness, waypoints))

    elite_size = max(5, int(pop_size * 0.1))

    def tournament_select(sorted_population, k=3):
        idx = np.random.randint(0, len(sorted_population), size=k)
        candidates = [sorted_population[i] for i in idx]
    ...
    for gen in range(iters):
        ...
        if getattr(params, 'print_ga_best_each_iter', False):
            ...
            if gen % interval == 0:
                _, best_dbg = calculate_path_fitness(population[0][1], start_pos, target_pos, obstacles, params, base_offsets)
                print(
                    f"[GA-BEST] iter={gen:03d} J={best_dbg['J']:.6f} "
                    f"L={best_dbg['L']:.2f} S={best_dbg['S']:.2f} R={best_dbg['R']:.2f} "
                    f"W={best_dbg['W']:.4f} F={best_dbg['F']:.2f}"
                )

        if params.debug_fitness and (gen % 5 == 0):
            _, dbg = calculate_path_fitness(population[0][1], start_pos, target_pos, obstacles, params, base_offsets)
            print(
                f"[GA] gen={gen:02d} J={dbg['J']:.4f} "
        ...
        # 3) 交叉 + 变异生成后代
        mutation_sigma = 40 * (1.0 - gen / max(1, iters))
        mutation_prob = 0.1
        ...
        while len(next_population) < pop_size:
            ...
            for w in range(n_waypoints):
                if np.random.rand() < mutation_prob:
                    child[w] += np.random.normal(0, mutation_sigma, size=2)

            child_fitness = evaluate(child)
            next_population.append((child_fitness, child))

        population = next_population[:pop_size]

    population.sort(key=lambda x: x[0])
    candidate_paths = [item[1] for item in population]
...
def run_point_to_point_formation():
    ...
    print("正在执行全局 遗传算法 (GA) 路径规划...")
    global_waypoints = ga_global_route_adjustment(
        start_pos,
        target_pos,
        obstacles,
        params,
    ...
    csv_filename = 'uav_states_ga_apf.csv'
    print(f"开始仿真并记录数据到 {csv_filename} (混合架构：Global GA + Local APF)...")
    ...
    ax.set_title('Multi-UAV Formation (Global GA + Local APF)')
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ...
    gif_name = 'uav_formation_p2p_ga_apf_static.gif'
