...
def al_global_route_adjustmen(start_pos, target_pos, obstacles, params, pop_size=100, iters=50, n_waypoints=20, base_offsets=None):
    ...
    def sample_cauchy(loc, scale):
        return loc + scale * np.tan(np.pi * (np.random.rand() - 0.5))
    ...
    for gen in range(iters):
        ...
        F = np.zeros(np_current, dtype=float)
        for i in range(np_current):
            while F[i] <= 0.0:
                F[i] = sample_cauchy(MF[mem_slots[i]], 0.1)
        F = np.clip(F, 0.0, 1.0)
        ...
        for i in range(np_current):
            ...
            below = mutant < lower_bound
            above = mutant > upper_bound
            mutant[below] = 0.5 * (lower_bound[below] + X[i, below])
            mutant[above] = 0.5 * (upper_bound[above] + X[i, above])
            V[i] = mutant
        ...
        for i in range(np_current):
            ...
            if trial_fitness[i] < fitness[i]:
                ...
                if archive.shape[0] < archive_cap:
                    archive = np.vstack([archive, child])
                    archive_fitness = np.append(archive_fitness, child_fit)
                else:
                    replace_idx = np.random.randint(0, archive_cap)
                    archive[replace_idx] = child
                    archive_fitness[replace_idx] = child_fit
...
def run_point_to_point_formation():
    ...
    csv_filename = 'uav_states_alshade_apf.csv'
    ...
    ax.plot(global_waypoints[:, 0], global_waypoints[:, 1], 'ko--', linewidth=1.5, alpha=0.5, label='Global Route (AL-SHADE)')
    ...
    gif_name = 'uav_formation_p2p_alshade_apf_static.gif'
