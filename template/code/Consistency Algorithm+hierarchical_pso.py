...
class UAVParams:
    def __init__(self):
        ...
        # PSO 初始化带宽参数（基线附近按上下界区间均匀扰动）
        self.pso_waypoint_init_sigma = 50.0
...
def compute_risk_components(points, obstacles, params, base_offsets):
    ...
    for k in range(len(points) - 1):
        ...
        pop_size = segment_sample_points(a, b, params.path_sample_step)
            ...
            for i in range(n_uav):
                for j in range(i + 1, n_uav):
                    ...
                    if d_ij <= params.comm_range:
                        ...
                        delay_pop_size.append(delay_ij)
                        ...
                        delay_pop_size.append(params.comm_delay_base + params.comm_delay_dist_gain * params.comm_range * 1.5)
...
def compute_reliability_cost(turn_angles, clearances, delay_pop_size, connected_pairs, total_pairs, params):
    if turn_angles:
    ...
    if delay_pop_size:
        delay_arr = np.asarray(delay_pop_size, dtype=float)
...
def pso_global_route_adjustment(start_pos, target_pos, obstacles, params, pop_size=100, iters=50, n_waypoints=20, base_offsets=None):
    """全局 PSO 路径规划：输出起始点到目标点之间的 N 个空间航点 (x, y)。"""
    if not obstacles:
    ...
    init_sigma = float(getattr(params, 'pso_waypoint_init_sigma', 50.0))
    ...
    particles = np.zeros((pop_size, n_waypoints, 2), dtype=float)
    for i in range(pop_size):
        ...
        particles[i] = np.clip(init_waypoints, waypoint_lower, waypoint_upper)

    velocities = np.zeros_like(particles)

    personal_best = particles.copy()
    personal_cost = np.full(pop_size, float('inf'))

    global_best = np.zeros((n_waypoints, 2))
    ...
    for t in range(iters):
        for i in range(pop_size):
            ...
            if cost < personal_cost[i]:
                personal_cost[i] = cost
                personal_best[i] = particles[i].copy()
            if cost < global_cost:
                global_cost = cost
                global_best = particles[i].copy()

        r1 = np.random.rand(pop_size, n_waypoints, 1)
        r2 = np.random.rand(pop_size, n_waypoints, 1)
        ...
        print(f"PSO 迭代 {t+1}/{iters} - 当前全局最佳成本: {global_cost:.4f}")
...
def run_point_to_point_formation():
    ...
    print("正在执行全局 PSO 路径规划...")
    global_waypoints = pso_global_route_adjustment(
        start_pos,
        target_pos,
        obstacles,
        params,
    ...
    csv_filename = 'uav_states_pso_apf.csv'
    print(f"开始仿真并记录数据到 {csv_filename} (混合架构：Global PSO + Local APF)...")
    ...
    ax.plot(global_waypoints[:, 0], global_waypoints[:, 1], 'ko--', linewidth=1.5, alpha=0.5, label='Global Route (PSO)')
    ...
    ax.set_title('Multi-UAV Formation (Global PSO + Local APF)')
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ...
    gif_name = 'uav_formation_p2p_pso_apf_static.gif'
