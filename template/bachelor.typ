...
#show: nwpu-thesis.with(
  ...
  abstract: [
    随着无人机技术的快速发展，无人机集群在城市低空物流、环境监测、灾害救援及军事侦察等领域的应用日益广泛。然而，在复杂多障碍物环境下，实现无人机集群的安全、高效路径规划仍存在巨大挑战。传统启发式算法如粒子群算法（PSO）、蚁群算法（ACO）、遗传算法（GA）、人工蜂鸟算法（AHA）、螳螂搜索算法（MSA）以及 AL-SHADE 算法在二维高密度障碍环境中的性能存在差异，传统算法容易出现路径不连续、局部收敛或不可行解等问题，而人工势场法在复杂环境下容易陷入局部极小值或死锁。为提升路径规划的全局搜索能力、轨迹连续性及编队稳定性，本文提出了一种结合多智能体编队控制、全局优化与局部避障的分层耦合路径规划方法，并在统一适应度函数和分层控制框架下对 AL-SHADE 算法进行了改进。本文在全局路径规划实验中复现了 PSO、ACO、GA、AHA、MSA 和原生 AL-SHADE 算法以进行性能对比，同时提出改进的 AL-SHADE 算法，其主要改进策略包括外部档案相似度去重机制、威胁驱动的 Lévy-高斯双态变异缩放因子以及 B 样条曲线参数化路径建模，通过路径连续性建模、环境威胁感知与档案优化提升了路径平滑性、集群编队保持能力及机间安全裕度。在局部避障策略上，本文在传统人工势场法基础上引入切向旋转势场力，使无人机能够沿障碍边缘稳定绕行，并与全局路径及编队控制相耦合，从而提升局部机动性与避障能力。通过构建二维多障碍物仿真环境，将编队控制层、全局规划层与局部避障层进行分层耦合验证，并采用路径长度、平滑度、规划成功率、收敛迭代次数以及队形保持误差等指标对方法性能进行评价。实验结果表明，改进的 AL-SHADE 算法在二维高密度障碍物环境下能够显著提升无人机集群的路径平滑性、编队稳定性及机间安全裕度，相比其他算法表现出更优的全局规划与局部避障性能。
  ],
...
在无人机集群路径规划领域，国外研究起步较早，在多无人机协同控制、智能优化算法以及复杂环境避障等方面形成了较为系统的理论与方法体系。围绕多无人机在二维或三维多障碍环境下的安全飞行任务，相关研究主要集中于编队控制与协同飞行、全局路径规划以及局部避障三个方向。

*编队控制与协同飞行。* 多无人机编队控制的核心在于通过局部信息交互实现整体队形的稳定形成与协同跟踪。Ren 和 Beard 系统阐述了多智能体一致性控制理论及其在多飞行器协同控制中的应用，为无人机集群分布式控制提供了重要理论基础@RenBeard2008。在此类方法中，通信拓扑通常由邻接矩阵和拉普拉斯矩阵描述，个体仅依赖邻居状态即可实现速度、位置或队形误差的一致收敛。随着任务场景逐渐从开阔空域扩展至狭窄通道、城市低空和动态障碍环境，虚拟领航者-跟随者结构、队形保持约束和自适应队形重构成为研究重点。相关研究表明，在障碍物密集或通道受限环境中，固定队形容易导致编队包络过大，因此需要根据通道宽度、通信链路和避障需求动态调整队形尺度与拓扑权重@Zhang2019Formation。

*全局路径规划。* 在全局路径规划方面，粒子群优化、蚁群算法和遗传算法等经典启发式算法被广泛用于连续空间和组合空间搜索。其中，Kennedy 和 Eberhart 提出的粒子群优化算法通过个体经验和群体经验共同引导搜索方向，具有参数少、收敛速度快等特点@Kennedy1995PSO。然而，在复杂多障碍物环境中，传统群体智能算法往往存在早熟收敛、路径贴近障碍物、航点分布不均以及不可行解比例较高等问题。为提升复杂连续空间下的优化能力，近年来研究者进一步引入新型生物启发式算法与自适应差分进化算法。例如，螳螂搜索算法通过模拟螳螂捕食与性食同类行为，在全局探索和局部开发之间建立分阶段搜索机制@MSA2023；L-SHADE 则在 SHADE 的成功历史参数自适应基础上引入线性种群规模缩减机制，提高了高维连续优化的收敛效率@Tanabe2014LSHADE。进一步地，AL-SHADE 将自适应 L-SHADE 框架应用于无人机集群资源配置问题，说明自适应差分进化思想在无人机群体优化任务中具有良好的扩展潜力@ALSHADE2022。

*局部避障与动态环境适应。* 局部避障研究主要面向未知障碍物、动态障碍物或全局路径偏差情况下的实时安全修正。Khatib 提出的人工势场法通过在目标点和障碍物周围构造引力场与斥力场，使机器人能够在连续空间中实时避障@Khatib1986APF。该方法结构简单、实时性强，因此被广泛应用于移动机器人和无人机避障任务。但经典 APF 在复杂障碍物环境中容易出现局部极小值、目标不可达和通道振荡等问题。为克服这些缺陷，国外研究通常通过动态势场、速度障碍、切向力场或局部重规划机制进行改进，并将局部避障与编队控制耦合，以保证集群在紧急避障时仍能维持基本队形和通信连通性。

综上所述，国外无人机集群路径规划研究已经形成了较为成熟的理论体系和丰富的算法谱系，但在二维高密度障碍物环境下，仍存在路径连续性不足、全局规划与局部避障耦合不充分、编队几何约束难以直接融入全局搜索等问题。这些问题为后续基于轨迹参数化和环境感知优化的改进算法提供了研究空间。

=== 国内研究现状

国内无人机集群路径规划研究近年来发展迅速，研究重点逐渐从单机路径规划扩展到多机协同、编队保持、复杂障碍物避障和分层仿真验证等方向。与国外侧重理论模型和通用算法框架不同，国内研究更加重视工程可实现性、算法集成和仿真场景构建。

*编队控制层研究。* 在编队控制方面，国内学者多以虚拟领航者模型和一致性控制理论为基础，结合无人机运动学约束、通信拓扑变化和任务队形需求，研究二维或三维空间中的队形保持问题。相关研究通常将“V”型、人字型或横队队形作为典型队形，通过设计位置误差、速度误差和邻居协同项，使僚机能够围绕领航者保持期望相对位置@ZhangW2020UAVFormation。针对狭窄通道或局部避障场景，国内研究进一步关注队形压缩、队形恢复和横向偏置调节等机制，使编队能够在保持整体结构的同时通过受限空间。

*全局路径规划层研究。* 在全局路径规划方面，国内研究者广泛采用 PSO、ACO、GA 等经典智能优化算法，并围绕无人机路径规划特点进行适应性改进。常见改进包括构造包含路径长度、障碍物风险、平滑度和约束惩罚的综合适应度函数，利用禁忌表或惩罚函数降低不可行路径比例，以及通过坐标变异、精英保留和局部搜索提高算法收敛质量@Wang2021APF。随着新型群体智能算法的发展，MSA、AHA、AL-SHADE 等算法也逐渐被尝试用于无人机路径规划任务。然而，二维航点编码通常会带来路径折线明显、搜索维度较高和轨迹连续性不足等问题，因此 B 样条、Bezier 曲线以及分段多项式轨迹等参数化方法被用于降低优化维度并提升路径可飞行性@Liu2020UAVPath。

*局部避障与动态避碰研究。* 在局部避障方面，国内研究重点关注密集障碍物环境中的实时避碰和局部死锁问题。改进人工势场法仍是工程应用中较常用的局部避障方法之一，其改进方向包括斥力函数重构、切向旋转力引入、动态权重调整以及与全局引导点结合等。相关研究表明，在“U”型障碍物、狭窄通道或多障碍物密集分布场景下，单纯径向斥力容易使无人机出现振荡或停滞，而引入切向绕行引导和速度约束校正能够有效提高局部避障成功率@Wang2021APF。对于多无人机集群而言，局部避障还需要与编队控制同时作用，避免单机避障行为破坏整体队形或造成机间距离过近。

总体来看，国内无人机集群路径规划研究已形成以工程应用和算法改进为导向的研究体系，但在高密度障碍物环境中仍存在若干不足：一是全局路径规划与局部避障之间的接口关系尚不够紧密；二是路径平滑性、编队保持和安全裕度往往分别处理，缺少统一评价与协同优化机制；三是部分智能优化算法直接应用于航点编码时容易产生折线轨迹和不可执行路径。因此，构建兼顾全局搜索、路径连续性、局部避障和编队稳定性的分层耦合规划框架，仍具有较高研究价值。

=== 国内外研究比较与评价

从总体发展趋势看，国外研究在一致性控制理论、群体智能优化算法和实时避障模型方面积累较深，形成了较完整的方法体系；国内研究则更加关注工程化实现、算法改进和多模块集成验证。二者共同推动了无人机集群路径规划由单一最短路径搜索向多目标、多约束和分层耦合优化方向发展。然而，现有研究仍存在以下不足。

第一，经典群体智能算法在复杂障碍物环境中的全局搜索能力与局部开发能力难以兼顾。PSO 收敛速度较快，但在障碍物密集区域容易过早聚集于局部优良通道，导致路径安全裕度不足；ACO 和 GA 具有较强的随机搜索能力，但在连续高维航点空间中容易出现收敛速度慢、路径曲折或不可行解比例较高等问题。第二，新型生物启发式算法虽然能够增强全局探索能力，但其参数机制通常与具体环境风险缺少直接关联，难以根据障碍物分布和个体受威胁程度动态调整搜索步长。第三，现有路径规划方法多将全局路径搜索、局部避障和编队控制分开设计，导致优化过程中难以同时兼顾路径长度、轨迹平滑性、编队保持和机间安全裕度。尤其在二维高密度障碍物环境中，离散航点编码生成的折线路径往往不具备良好的连续性，后续局部避障和编队控制需要进行较大幅度修正，从而影响整体飞行稳定性。

相比上述方法，AL-SHADE 作为一种基于成功历史记忆的自适应差分进化算法，具备较强的连续空间寻优能力。其通过历史成功参数记忆、自适应缩放因子与交叉概率更新以及线性种群规模缩减机制，能够在搜索早期保持较好的种群多样性，在后期逐步增强局部开发能力。因此，AL-SHADE 更适合用于无人机路径规划中连续航点坐标的全局优化问题。但原生 AL-SHADE 直接应用于无人机集群路径规划时仍存在三方面不足：一是采用离散航点编码时容易产生折线轨迹，路径平滑性不足；二是变异参数主要依据历史成功经验更新，缺少对障碍物威胁程度的环境感知；三是外部档案可能积累大量相似个体，使档案均值引导方向退化，降低搜索多样性。

基于上述分析，本文选择 AL-SHADE 作为主要全局路径规划算法框架，并在统一适应度函数和分层控制框架下对其进行改进：首先，引入 B 样条曲线参数化方法，将离散航点序列转化为连续光滑轨迹，提高路径可执行性；其次，构建基于障碍物净空距离的种群威胁度模型，并将其反馈至 Lévy-Gaussian 双态变异缩放因子生成机制，使算法能够根据个体所处环境风险自适应调整搜索步长；最后，设计外部档案相似度去重机制，减少冗余历史解对档案均值的干扰，增强 current-to-Amean/1 变异策略的引导有效性。通过上述改进，本文旨在提升 AL-SHADE 在二维高密度障碍物环境下的路径平滑性、编队保持能力和机间安全裕度。
