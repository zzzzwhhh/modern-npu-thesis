...
#show: nwpu-thesis.with(
  ...
  abstract: [
    面向城市低空物流、环境监测、灾害救援和军事侦察等任务，无人机集群需要在障碍物密集的空间中完成协同飞行。对这类任务而言，单条路径可行并不等于集群飞行安全：领航者轨迹过于折线会增加僚机跟踪误差，局部避障若与编队控制脱节，也可能造成队形被拉散或机间距离过小。因此，二维多障碍物环境下的集群路径规划，需要同时处理路径长度、轨迹连续性、编队稳定性和安全裕度等问题。

    本文围绕上述问题，构建了一个由编队控制、全局路径规划和局部避障组成的分层耦合框架。全局规划层复现粒子群算法（PSO）、蚁群算法（ACO）、遗传算法（GA）、人工蜂鸟算法（AHA）、螳螂搜索算法（MSA）和原生 AL-SHADE 算法，用作横向对比。在此基础上，针对原生 AL-SHADE 直接用于航点优化时存在的档案冗余、环境风险感知不足和路径连续性较弱等问题，本文引入外部档案相似度去重机制、威胁驱动的 Lévy-高斯双态变异缩放因子以及 B 样条曲线参数化路径建模。三项改进分别作用于历史解利用、搜索步长调节和轨迹连续化，使算法在障碍物密集区域中能够更稳地搜索可行路径。

    在局部避障部分，本文以传统人工势场法（APF）为基础加入切向旋转势场力，引导无人机沿障碍物边缘绕行，而不是只依赖径向斥力进行避让。局部避障指令与全局路径跟踪、编队一致性控制共同作用，使无人机在靠近障碍物时仍能尽量维持任务方向和队形结构。

    仿真部分构建了二维多障碍物环境，并从路径长度、平滑度、规划成功率、收敛迭代次数和队形保持误差等指标评价各算法表现。结果显示，改进后的 AL-SHADE 在路径平滑性、编队稳定性和机间安全裕度方面均有较好表现，能够在全局规划质量和局部避障效果之间取得更均衡的结果。
  ],
...
随着无人机技术不断成熟，无人机已经进入城市低空物流配送、环境监测、灾害救援和军事侦察等应用场景。与单架无人机相比，无人机集群（UAV Swarm）可以依靠协同控制提高任务效率和系统鲁棒性，也更适合执行复杂作业任务。但在实际飞行中，环境约束会明显增加集群控制难度；当场景中分布大量障碍物时，路径规划问题会变得更加复杂。

粒子群算法（PSO）、蚁群算法（ACO）和遗传算法（GA）等传统路径规划方法，在连续空间全局优化中已有较多应用。但这些方法直接用于二维高密度障碍物环境时，容易出现路径不连续、局部收敛或早熟收敛等问题。例如，PSO 在靠近障碍物边缘时可能产生“贴墙飞”现象，ACO 在密集障碍区域中可能陷入循环搜索，GA 在交叉重组过程中也可能生成穿越障碍物的不可行解。经典人工势场法（APF）虽然实时性较好，但在“U 型”障碍物或狭窄通道附近，仍可能使无人机集群陷入局部极小值或死锁。

近几年，生物启发式算法和自适应优化算法在连续优化问题中表现出较强的全局搜索能力。螳螂搜索算法（MSA）模拟螳螂捕食中的突击和猎物追踪行为，可用于部分多峰值连续函数优化问题；但其原生形式用于二维带障碍空间时，仍存在随机初始化不均和早熟收敛等不足。AL-SHADE 算法通过成功历史参数自适应机制和线性种群规模缩减策略，提高微分进化算法在高维连续优化问题中的搜索效率。不过，若将其直接用于二维离散路径规划，离散航点之间缺少连续性约束，容易造成无人机航迹不够平滑。

因此，二维多障碍物环境下的无人机集群路径规划研究，既需要关注全局路径的最优性和连续性，也需要考虑集群编队稳定性与局部动态避障能力。围绕这些问题开展研究，有助于完善无人机集群协同规划方法，也能为复杂环境下的自主飞行任务提供参考。

== 研究现状

=== 国外研究现状

国外关于无人机集群路径规划的研究起步较早，相关工作主要围绕多智能体协同控制、智能优化搜索和实时避障展开。经过多年发展，这一领域已经形成较为完整的方法体系。研究者既关注集群个体之间的信息交互和队形协同，也重视复杂障碍环境中路径搜索的效率与安全性。对于二维或三维多障碍场景中的飞行任务，国外研究通常不只关注“能否找到一条路径”，还会进一步考虑队形如何保持、路径是否连续可飞，以及遇到局部障碍时系统能否及时修正。

编队控制与协同飞行。 在编队控制方面，国外学者较早将多智能体一致性理论用于多无人机协同飞行。Ren 和 Beard 对多智能体一致性控制理论及其在多飞行器协同控制中的应用作出较系统的论述，为后续无人机集群分布式控制研究提供重要基础@RenBeard2008。这类方法通常用邻接矩阵和拉普拉斯矩阵描述通信拓扑，使每架无人机依靠邻居状态信息，逐步实现速度、位置或队形误差的一致收敛。早期研究多面向开阔空域中的稳定队形保持，问题边界较清晰。随着应用场景扩展到城市低空、狭窄通道和动态障碍环境，固定队形的局限逐渐显现。尤其在障碍物密集或通道宽度受限时，过大的编队包络可能直接导致路径不可行。因此，虚拟领航者-跟随者结构、队形约束调节和自适应队形重构逐渐成为研究重点。相关研究指出，集群需要根据通道宽度、通信链路状态和避障需求动态调整队形尺度与拓扑权重，才能在保持协同关系的同时降低碰撞风险@Zhang2019Formation。

*全局路径规划。* 全局路径规划方面，粒子群优化、蚁群算法和遗传算法等启发式算法在国外研究中应用较多。这些算法能够在连续空间或组合空间中搜索可行解，因此很早就被用于移动机器人和无人机路径规划任务。其中，Kennedy 和 Eberhart 提出的粒子群优化算法利用个体经验和群体经验共同引导搜索方向，具有参数较少、实现简单、收敛速度较快等特点@Kennedy1995PSO。不过，一旦环境中障碍物分布较密，传统群体智能算法的缺陷也比较明显：种群可能过早聚集在局部较优区域，航点分布也可能不均匀，甚至出现路径贴近障碍物或不可行解比例偏高的情况。为改善这些问题，研究者逐渐引入新型生物启发式算法和自适应差分进化算法。螳螂搜索算法通过模拟螳螂捕食与性食同类行为，将全局探索和局部开发划分为不同搜索阶段@MSA2023；L-SHADE 在 SHADE 成功历史参数自适应的基础上加入线性种群规模缩减机制，使高维连续优化中的收敛效率得到提高@Tanabe2014LSHADE。进一步地，AL-SHADE 被用于无人机集群资源配置问题，表明自适应差分进化思想并不只适用于标准函数优化，也具有向无人机群体优化任务扩展的潜力@ALSHADE2022。

*局部避障与动态环境适应* 相比全局规划，局部避障更看重实时响应能力。无人机在执行全局航路时，可能遇到未知障碍物、局部路径偏差或环境变化，系统需要在较短时间内给出安全修正。Khatib 提出的人工势场法在目标点和障碍物周围分别构造引力场与斥力场，使机器人能够在连续空间中实现实时避障@Khatib1986APF。该方法结构直观，计算量较小，所以在移动机器人和无人机避障研究中一直被广泛采用。但问题也很清楚：经典 APF 在复杂障碍物环境中容易出现局部极小值、目标不可达和通道振荡等现象，特别是在凹形障碍物或狭窄通道附近，单纯依赖径向斥力并不稳定。为此，国外研究通常从动态势场、速度障碍、切向力场和局部重规划等方向对其进行改进。有些研究还将局部避障与编队控制结合起来，使集群在紧急避障时不至于完全打散队形，同时尽量维持基本通信连通性。

总体来看，国外无人机集群路径规划研究已经积累较成熟的理论基础，也形成多种算法类型。但在二维高密度障碍物环境中，这些方法仍存在一些问题。路径连续性不足、全局规划与局部避障衔接不够紧密、编队几何约束难以直接融入全局搜索过程，都是较常见的限制因素。因此，利用轨迹参数化方法改善路径连续性，并在优化过程中引入环境威胁感知，仍有进一步研究的空间。

=== 国内研究现状

近几年，国内关于无人机集群路径规划的研究推进较快。研究对象不再局限于单机路径搜索，而是逐步转向多机协同、编队保持、复杂障碍物避障和分层仿真验证等问题。相比国外更偏重理论模型和通用算法框架的研究思路，国内相关工作更关注方法在具体场景中的可落地性，包括算法如何集成、仿真环境如何构建，以及工程实现中是否便于调试和复现。

*编队控制层研究。* 在编队控制方面，国内学者多以虚拟领航者模型和一致性控制理论为基础，将无人机运动学约束、通信拓扑变化和任务队形需求一起纳入考虑，研究二维或三维空间中的队形保持问题。相关研究常以“V”型、人字型或横队作为典型编队形式，并通过位置误差、速度误差和邻居协同项来约束僚机运动，使其围绕领航者保持期望相对位置@ZhangW2020UAVFormation。面对狭窄通道或局部避障场景时，固定队形的适应性有限，队形压缩、队形恢复和横向偏置调节等方法因此受到更多关注。这类机制可以让编队在保持基本结构的同时，通过宽度受限的区域。

*全局路径规划层研究。* 在全局路径规划方面，PSO、ACO、GA 等经典智能优化算法仍是国内研究中常见的选择。研究者通常会结合无人机路径规划特点对算法进行改造，例如构造包含路径长度、障碍物风险、平滑度和约束惩罚的综合适应度函数，借助禁忌表或惩罚函数减少不可行路径，再通过坐标变异、精英保留和局部搜索改善收敛质量@Wang2021APF。随着新型群体智能算法的发展，MSA、AHA、AL-SHADE 等方法也开始用于无人机路径规划任务。不过，二维航点编码本身存在明显限制：路径容易形成折线，搜索维度偏高，轨迹连续性也不够理想。为缓解这一问题，B 样条、Bezier 曲线和分段多项式轨迹等参数化方法被用于降低优化维度，并提升规划路径的可飞行性@Liu2020UAVPath。

*局部避障与动态避碰研究。* 在局部避障方面，国内研究主要关注密集障碍物环境中的实时避碰和局部死锁问题。改进人工势场法仍是工程应用中使用较多的一类方法，常见做法包括重构斥力函数、引入切向旋转力、调整动态权重，以及将局部避障目标与全局引导点结合。相关研究表明，在“U”型障碍物、狭窄通道或多障碍物密集分布场景中，单纯依靠径向斥力容易引发无人机振荡或停滞；加入切向绕行引导和速度约束校正后，局部避障成功率可以得到改善@Wang2021APF。对于多无人机集群而言，局部避障还需要和编队控制配合使用。若只关注单机避障，某架无人机的临时机动可能会破坏整体队形，甚至使机间距离过近。

整体来看，国内无人机集群路径规划研究已经形成以工程应用和算法改进为主的研究思路，但高密度障碍物环境中的一些问题仍然突出。全局路径规划与局部避障之间的接口关系还不够紧密，路径平滑性、编队保持和安全裕度也常被分开处理，统一评价和协同优化机制仍显不足。部分智能优化算法直接用于航点编码时，还容易生成折线轨迹或不可执行路径。因此，面向复杂障碍物场景，仍有必要构建一种兼顾全局搜索、路径连续性、局部避障和编队稳定性的分层耦合规划框架。

=== 国内外研究比较与评价

从整体发展脉络来看，国外研究在一致性控制理论、群体智能优化算法和实时避障模型方面积累较多，方法体系相对完整；国内研究则更强调工程实现、算法改进和多模块集成验证。两类研究共同说明，无人机集群路径规划已经不再只是单纯寻找最短路径，而是逐渐转向多目标、多约束和分层耦合的综合优化问题。不过，现有方法在复杂障碍物场景下仍有一些问题需要继续处理。

首先，经典群体智能算法很难同时兼顾全局搜索和局部开发。PSO 的收敛速度较快，但在障碍物密集区域容易过早聚集到局部较优通道，导致路径安全裕度不足；ACO 和 GA 的随机搜索能力较强，可是在连续高维航点空间中，二者又容易出现收敛速度慢、路径曲折或不可行解比例偏高等问题。其次，新型生物启发式算法虽然增强了全局探索能力，但其参数调节机制通常没有直接关联具体的环境风险，因而难以根据障碍物分布和个体受威胁程度动态改变搜索步长。还有一点也较为关键：不少路径规划方法仍将全局路径搜索、局部避障和编队控制分开设计，优化过程中很难同时考虑路径长度、轨迹平滑性、编队保持和机间安全裕度。尤其在二维高密度障碍物环境中，离散航点编码生成的折线路径连续性较差，后续局部避障和编队控制往往需要较大幅度修正，最终会影响集群飞行的稳定性。

与上述方法相比，AL-SHADE 是一种基于成功历史记忆的自适应差分进化算法，在连续空间寻优方面具有一定优势。该算法通过历史成功参数记忆、自适应缩放因子与交叉概率更新，以及线性种群规模缩减机制，使种群在搜索早期保持较好的多样性，并在后期逐步加强局部开发能力。因此，将 AL-SHADE 用于无人机路径规划中的连续航点坐标优化，具有较好的适配性。需要注意的是，原生 AL-SHADE 直接用于无人机集群路径规划时仍存在不足：离散航点编码容易生成折线轨迹，路径平滑性不够；变异参数主要依赖历史成功经验更新，对障碍物威胁程度的感知不足；外部档案中还可能累积大量相似个体，使档案均值的引导方向退化，进而削弱搜索多样性。

基于上述分析，本文选取 AL-SHADE 作为主要全局路径规划框架，并在统一适应度函数和分层控制框架下对其进行改进。具体来说，本文设计外部档案相似度去重机制，以减少冗余历史解对档案均值的干扰，增强 current-to-Amean/1 变异策略的引导有效性；同时，本文构建基于障碍物净空距离的种群威胁度模型，并将其反馈到 Lévy-Gaussian 双态变异缩放因子生成过程中，使算法能够根据个体所处环境风险自适应调整搜索步长；此外，本文引入 B 样条曲线参数化方法，将离散航点序列转化为连续光滑轨迹，从而提高路径可执行性。通过这些改进，本文希望提升 AL-SHADE 在二维高密度障碍物环境下的路径平滑性、编队保持能力和机间安全裕度。

== 研究内容

结合实际任务需求和前文分析的问题，本文以二维多障碍物环境下的无人机集群为研究对象，围绕编队控制、全局路径规划和局部动态避障展开研究。为使所提方法在复杂约束和密集障碍场景中具备可行性、稳定性与可复现性，本文将研究内容划分为以下四个方面：

1. 多无人机编队协同控制机理与算法设计： 本文构建基于虚拟领航者模型和多智能体一致性算法的协同控制框架，明确邻接矩阵与拉普拉斯矩阵对通信拓扑和收敛性的约束作用。针对二维平面中的编队保持与协同跟踪问题，本文设计位置误差和速度误差双耦合的控制律，并结合无人机速度、加速度和角速度等运动学约束进行物理可行性校验。在此基础上，本文进一步引入横向偏置自适应调节机制，使集群在开阔空间和局部受限空间内都能够稳定收敛到“V”型、人字型等预定队形，并在通道收缩和恢复过程中保持一定的队形弹性。
2. 多障碍空间下的全局路径智能优化与轨迹平滑： 面向密集障碍物和多约束条件下的二维环境，本文复现并改进粒子群（PSO）、蚁群（ACO）、遗传（GA）等经典启发式算法，通过禁忌表、路径可行性惩罚和坐标变异机制提高搜索效率与可行解比例。在此基础上，针对早熟收敛和高维路径编码问题，本文提出混沌初始化与非线性步长衰减的改进螳螂搜索算法（MSA）；针对差分进化算法用于路径离散编码时连续性不足的问题，本文引入 B 样条曲线参数化的 AL-SHADE 算法，实现路径维度降维和连续轨迹建模，从而提高全局规划结果的平滑性与可飞行性。
3. 基于改进人工势场的局部动态避障策略： 针对局部动态障碍以及“U”型障碍容易造成局部极小值的问题，本文在传统人工势场法（APF）的引力和斥力模型基础上引入切向旋转势场力，构建能够引导无人机沿障碍边缘绕行的局部避障机制。该机制与全局路径引导点和编队控制律进行耦合，使无人机在不明显偏离全局任务目标的前提下，能够在障碍边缘和狭窄通道中完成较稳定的局部机动与逃逸。
4. 多层算法集成与仿真验证体系： 本文构建统一的二维多障碍物仿真环境，将编队控制层、全局规划层和局部避障层进行分层耦合验证。通过设置典型障碍密度和障碍分布场景，本文采用路径长度、平滑度、规划成功率、收敛迭代次数和队形保持误差等指标进行横向对比，以验证所提方法在安全性、稳定性和效率方面的综合表现。
...
无人机集群路径规划涉及多无人机编队控制、群体智能全局规划、局部避障与轨迹平滑等多个关键技术环节。相关研究既包括多智能体一致性控制、虚拟领航者-跟随者模型等协同控制方法，也包括 PSO、ACO、GA、MSA、AL-SHADE 等群体智能优化算法，以及人工势场法、B 样条曲线等局部避障与路径参数化技术。本章围绕本文所采用的主要技术路线，对相关工作进行梳理，并分析现有方法与本文改进之间的关系。

== 多无人机编队控制

无人机集群的编队控制是实现协同飞行的基础，其核心目标是在保证单机运动约束的前提下，使多架无人机通过局部信息交互形成并维持期望队形。Olfati-Saber 等系统总结了一致性与协同控制在网络化多智能体系统中的基本理论，指出通信拓扑、邻接关系和拉普拉斯矩阵是影响多智能体收敛性的关键因素@OlfatiSaber2007Consensus。Ren 和 Beard 进一步从多飞行器协同控制角度对分布式一致性理论进行了系统阐述，为无人机集群编队控制提供了重要理论支撑@RenBeard2008。

在无人机编队任务中，虚拟领航者-跟随者模型被广泛用于描述集群整体运动与僚机相对队形之间的关系。该类方法通常将领航者作为全局任务参考，僚机依据期望相对位置、邻居状态和通信拓扑进行跟踪，从而实现“V”型、人字型或横队等队形的稳定收敛。国内相关研究在一致性控制框架基础上进一步引入横向偏置调节和队形自适应重构策略，以适应二维航迹规划和狭窄通道飞行任务@ZhangW2020UAVFormation。本文在此基础上建立二维平面内的多无人机编队控制模型，并结合速度、加速度和航向角速率约束，为后续全局路径规划和局部避障提供可执行的运动控制基础。

== 群体智能全局路径规划算法

全局路径规划用于在已知障碍物环境中搜索从起点到目标点的可行航迹，是无人机集群任务规划的核心环节。传统群体智能算法因其不依赖梯度信息、适用于非线性和多峰优化问题，被广泛应用于路径规划任务。Kennedy 和 Eberhart 提出的粒子群优化算法通过个体最优和群体最优共同引导搜索方向，具有结构简单、收敛速度快等优点@Kennedy1995PSO；Dorigo 和 Stützle 系统提出的蚁群优化方法通过信息素更新实现群体式路径搜索，在组合优化和图搜索问题中具有代表性@Dorigo2004ACO；遗传算法则通过选择、交叉和变异操作维持种群多样性，适合处理复杂搜索空间中的全局优化问题。

然而，在二维高密度障碍物环境中，传统 PSO、ACO 和 GA 容易出现早熟收敛、局部最优、路径曲折或不可行解比例较高等问题。近年来，新型群体智能算法和自适应差分进化算法逐渐被引入路径规划研究。螳螂搜索算法通过模拟螳螂捕食过程构建探索与开发机制，为复杂连续优化提供了新的搜索策略@MSA2023；L-SHADE 在 SHADE 成功历史参数记忆机制基础上引入线性种群规模缩减，提高了差分进化在高维连续优化中的收敛效率@Tanabe2014LSHADE；AL-SHADE 则进一步将自适应 L-SHADE 思想应用于无人机集群相关优化问题，表明该类算法适合处理无人机群体系统中的连续参数优化任务@ALSHADE2022。本文以 AL-SHADE 为主要全局路径规划框架，并针对无人机路径规划中的路径连续性、障碍威胁感知和档案多样性问题进行改进。

== 局部避障与动态环境适应

局部避障用于在全局路径执行过程中对突发风险或局部障碍进行实时修正。Khatib 提出的人工势场法通过目标引力和障碍物斥力构造虚拟势场，使机器人能够在连续空间中实现实时避障@Khatib1986APF。该方法计算简单、实时性强，因而被广泛用于移动机器人和无人机局部避障任务。但经典 APF 存在局部极小值、目标不可达以及狭窄通道振荡等问题，尤其在“U”型障碍物或多障碍物密集区域中容易产生死锁。

为缓解全局路径规划与局部控制之间的割裂，Quinlan 和 Khatib 提出了 Elastic Bands 方法，将路径规划结果与局部控制过程相连接，使路径能够在障碍物约束下进行弹性形变@Quinlan1993ElasticBands。后续研究在此基础上提出动态势场、切向旋转力和速度约束校正等改进策略，使无人机能够沿障碍物边缘绕行，降低局部死锁概率。本文采用改进 APF 局部避障方法，在传统引力与斥力基础上引入切向旋转势场力，并与全局路径引导点和编队控制律耦合，以实现障碍边缘和狭窄通道中的稳定机动。

== 轨迹参数化与技术整合

对于无人机路径规划而言，仅获得一组离散航点并不足以保证路径可执行性。若直接连接离散航点，容易产生折线、急转和曲率突变，增加飞行控制难度并影响编队稳定性。因此，B 样条曲线、Bezier 曲线和分段多项式轨迹常被用于路径平滑和轨迹参数化。B 样条曲线具有局部支撑性、连续性高和端点可约束等特点，适合将有限控制点转化为连续光滑轨迹，从而提升无人机航迹的可飞行性。

综上，现有研究分别在编队控制、全局路径规划、局部避障和轨迹平滑方面取得了较多成果，但复杂障碍物环境下仍需要将上述技术进行统一集成。本文在多层算法框架下，将一致性编队控制、AL-SHADE 全局优化、B 样条路径参数化和改进 APF 局部避障相结合，并通过统一适应度函数同时评价路径长度、平滑性、风险约束、通道宽度和队形保持效果，从而为无人机集群在二维多障碍物环境中的安全协同飞行提供完整解决方案。


= 模型构建与算法实现

无人机集群路径规划不仅需要获得一条从起点到目标点的可行航迹，还需要保证多架无人机在沿路径飞行过程中能够保持稳定队形，并在障碍物环境中具备协同避障能力。因此，在进行全局路径规划算法设计之前，有必要先建立无人机编队的运动描述和队形约束模型，明确无人机之间的相对位置关系、通信拓扑以及一致性控制机制。基于上述考虑，本节首先对无人机编队控制模型进行建模，为后续全局路径规划、局部避障和联合仿真提供基础。
...
通过一致性原理，本节分别构建了无人机在横向与纵向上的控制策略。在此基础上，通过将无人机的物理机动限制与控制律相融合，最终推导出了本文所提的协同控制算法。
...
控制器以周期 $Delta t$ 迭代运行：每步采集邻机延迟状态，按@eqt:improved-consensus-x、式@eqt:improved-consensus-y 计算虚拟加速度，经上述两级约束映射后输出自驾仪速度指令，驱动无人机状态更新。该架构在二维平面上为集群提供了兼顾拓扑鲁棒性与飞行安全性的编队控制能力，为后续全局路径规划奠定了载体平台层的协同基础@tao2023。
...
粒子更新后将其位置裁剪至航点搜索边界 $[bold(w)_j^(min), bold(w)_j^(max)]$ 内。若更新后的适应度优于个体历史最优，则更新 $bold(P)_i^("best")$；若进一步优于全局最优，则更新 $bold(G)^("best")$，从而逐步引导种群向全局最优路径收敛。算法终止条件为达到最大迭代次数 $T_("max")$。
...
前文分别给出了编队控制模型、全局路径规划算法和 APF 局部避障方法。三者是在同一任务闭环中承担不同层级的功能：全局规划层负责从起点到目标点生成可行的参考航路，编队控制层负责将全局航路转化为多无人机协同飞行过程中的队形保持指令，局部避障层则在执行过程中根据近场障碍物信息对控制指令进行实时修正。通过这种分层耦合方式，系统能够同时兼顾全局路径质量、编队稳定性和局部飞行安全。

具体而言，任务开始后首先输入起点、目标点、障碍物分布、无人机数量和期望编队构型等先验信息。全局路径规划算法在统一适应度函数约束下搜索最优航点序列，得到面向领航者的全局参考路径。随后，领航者沿该参考路径逐步跟踪当前目标航点，其余跟随者根据期望相对位移、邻接拓扑和一致性控制律生成编队保持控制量，从而使无人机集群在整体运动过程中维持预设队形。

在实际飞行执行阶段，局部避障层以更高的控制频率嵌入编队控制回路。当无人机预测轨迹进入障碍物影响范围时，APF 模块根据障碍物距离、径向斥力和切向绕行力计算安全修正量；当周围环境安全时，局部避障项趋近于零，系统主要由全局路径跟踪和编队保持控制主导。最终控制指令仍按照@eqt:integrated-control 进行合成，即由编队一致性控制量、全局导航加速度和局部避障修正量共同决定。该形式保证了局部避障不会完全替代全局路径规划，而是在保持任务方向的前提下提供必要的安全偏置。

因此，完整任务流程可以概括为"全局规划给出方向、编队控制保持结构、局部避障修正风险"。全局规划层决定路径的宏观质量，直接影响路径长度、到达时间和平滑性；编队控制层决定多机协同过程中的队形误差和机间安全裕度；局部避障层则决定集群面对障碍物扰动时的实时可行性。后续算法改进主要作用于全局规划层，但其效果会通过参考路径质量进一步传递到编队控制和局部避障过程，进而影响整个无人机集群任务的综合性能。
...
原生 AL-SHADE 在路径规划中的核心困境可归结为三点。第一，外部档案 $cal(A)$ 的随机替换策略可能存入大量重复或高度相似的解，造成档案多样性的隐性退化，削弱 current-to-Amean/1 变异策略所依赖的种群分布广度。第二，变异因子 $F$ 的柯西采样对全体个体一视同仁，无法感知障碍物的空间分布——在开阔区域需要的小步长精细搜索与在障碍物密集区域需要的大步长跳跃逃离之间缺少自适应切换机制。第三，航点序列的离散编码方式天然不具备路径连续性约束——各航点坐标独立演化，即使相邻两代在搜索空间中接近，其解码后的路径仍可能出现折线式的剧烈拐弯，导致路径平滑性项 $S$ 的优化与长度项 $L$ 的优化相互冲突。针对上述问题，TALG（Threat-Adaptive Lévy-Gaussian）框架依次引入基于欧氏距离的外部档案相似度去重、种群威胁度驱动的 Lévy-Gaussian 双态 $F$ 生成以及 B 样条曲线参数化路径建模三项改进，以分别提升档案多样性、环境感知搜索能力和路径几何连续性。
