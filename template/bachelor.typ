...
无人机集群路径规划涉及编队控制、全局路径搜索和局部避障等多个环节，其中全局规划算法决定了参考航迹的整体质量，也是后续编队跟踪和局部修正的基础。因此，本章重点围绕现有的全局路径规划算法展开，梳理其评价函数、路径编码方式以及典型智能优化算法的实现思路。

== 全局路径规划算法设计与实现
全局路径规划层需要为领航者生成一条从起点到目标点的参考路径，并使该路径能够被后续编队控制和局部避障模块稳定使用。由于不同优化算法产生的候选路径在长度、平滑性、安全裕度和编队可行性等方面存在差异，必须先建立统一的评价标准，用于衡量候选路径的优劣，并为 PSO、ACO、GA、MSA、AHA 以及 AL-SHADE 等算法提供共同的优化目标。因此，本节首先构建适应度函数，再依次给出各类全局路径规划算法的设计与实现。
=== 适应度函数设计
