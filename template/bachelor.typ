...
phi(d) = cases((1 - d / d_("safe"))^2, d < d_("safe"), 0, d >= d_("safe"))
$
...
在路径规划任务中，PSO 的优势是结构简单、收敛速度快，能够较快获得可行路径。但其搜索过程依赖全局最优粒子的牵引，在障碍物密集区域容易过早聚集，使种群多样性下降。因此，本文主要将 PSO 作为基础对比算法，用于衡量其他算法在收敛速度和路径质量上的差异。
...
连续蚁群算法能够利用优质解附近的概率分布进行局部搜索，也能通过档案维持一定多样性。但当档案过早集中在局部区域时，高斯采样范围会逐渐收缩，后期改进能力有限。因此，在本文实验中，ACO 主要用于观察基于解档案采样的连续优化方法在无人机路径规划中的表现。
...
$
sigma(g) = sigma_0 lr(1 - frac(g, G_("max")))
$
...
$
bold(x)_i^(t+1) = cases(
  bold(x)_i^t + bold(tau)_1 dot (bold(x)_i^t - bold(x)_a^t) + |tau_2| dot bold(U) dot (bold(x)_a^t - bold(x)_b^t), r_1 <= r_2,
...
$
bold(x)_i^(t+1) = cases(
  bold(x)_i^t + alpha dot (bold(x)_("ar")^t - bold(x)_a^t), r_9 <= r_(10),
...
$
(V T)_(i,j)^0 = cases("null", i = j, 0, i != j)
$
...
若蜂鸟位置得到改善，则其他蜂鸟对该新食物源的访问间隔也需要相应刷新，使新产生的优质解更容易在后续迭代中被关注。综上，AHA 的路径规划流程为：初始化蜂鸟种群和访问表；在每轮迭代中随机选择飞行模式，并在引导觅食与领地觅食之间切换；生成候选解后进行边界处理、适应度评价和贪婪选择；按规则更新访问表，并周期性执行迁徙觅食。该算法通过访问表减少重复搜索，通过多种飞行模式调节更新尺度，并通过迁徙机制维持全局探索能力。
...
其中 $bold(x)_("pbest")$ 从前 $p$% 优质个体中随机选取，$bold(x)_(r_2)$ 可来自当前种群或外部档案。第二类为 current-to-Amean/1：
...
$
u_(i,j) = cases(v_(i,j), r_j <= (C R)_i " or " j = j_("rand"), x_(i,j), "otherwise")
$

其中 $j_("rand")$ 保证至少一个维度来自变异向量。若变异向量越界，则采用父代与边界之间的中点进行修正：

$
v_(i,j) = cases((x_(i,j) + L_j) / 2, v_(i,j) < L_j, (x_(i,j) + U_j) / 2, v_(i,j) > U_j, v_(i,j), "otherwise")
$

选择阶段采用贪婪准则：

$
bold(x)_i^(g+1) = cases(bold(u)_i^g, J(bold(u)_i^g) <= J(bold(x)_i^g), bold(x)_i^g, "otherwise")
$
...
为提高后期开发能力，AL-SHADE 还采用线性种群规模缩减策略：

$
P_g = "round" lr(P_("init") - frac("FEs", "MaxFEs") (P_("init") - P_("min")))
$
