...
#show: nwpu-thesis.with(
  ...
  abstract: [
    ...
    本文围绕上述问题，构建了一个由编队控制、全局路径规划和局部避障组成的分层耦合框架。全局规划层复现粒子群算法（PSO）、蚁群算法（ACO）、遗传算法（GA）、人工蜂鸟算法（AHA）、螳螂搜索算法（MSA）和传统 AL-SHADE 算法，用作横向对比。在此基础上，针对 AL-SHADE 直接用于航点优化时存在的档案冗余、环境风险感知不足和路径连续性较弱等问题，本文引入外部档案相似度去重机制、威胁驱动的 Lévy-高斯双态变异缩放因子以及 B样条曲线参数化路径建模。三项改进分别作用于历史解利用、搜索步长调节和轨迹连续化，使算法在障碍物密集区域中能够更稳地搜索可行路径。
...
在已有的路径规划研究中，$A^*$、Dijkstra 和 RRT 等传统全局规划算法使用较早，也比较有代表性。Dijkstra 算法能够在图结构中求得确定意义下的最短路径，但搜索范围较大，计算开销会随着节点数量增加而迅速上升；$A^*$ 在 Dijkstra 的基础上加入启发函数，搜索效率有所提高，不过路径质量很依赖栅格分辨率和启发函数设计；RRT 适合在连续空间中快速扩展随机树，对高维空间比较友好，但生成路径往往不够平滑，后期还需要额外修剪和优化。对于二维高密度障碍物环境下的无人机集群任务来说，这类方法虽然思路清晰，但在路径平滑性、编队约束和多目标综合评价方面仍显得不够灵活。

在此基础上，粒子群算法（PSO）、蚁群算法（ACO）和遗传算法（GA）这一类传统方法，凭借其原理简单、易于实现的特点，在连续空间的全局优化问题上已经获得了相当广泛的应用。然而， 当它们被直接迁移到二维高密度障碍物环境下的集群路径规划时， 这些方法各自的局限性便开始集中暴露出来，往往容易表现出路径不连续、局部收敛或早熟收敛等问题。具体来看， PSO 在靠近障碍物边缘时， 由于粒子的速度更新机制缺乏对障碍物几何信息的有效利用，容易产生“贴墙飞”现象，使得规划出的路径紧贴着障碍物表面蜿蜒前行，既不安全， 也不利于实际飞行的跟踪控制；ACO 在障碍物分布密集的区域中，由于信息素的累积和挥发之间的反馈失衡， 蚂蚁个体极易在局部区域内来回打转，陷入循环搜索而迟迟无法收敛到可行路径； 而 GA 在交叉重组的过程中，因其染色体的离散编码方式与连续路径空间之间的不匹配，有可能会频繁生成直接穿越障碍物的不可行解， 这无疑增加了后续修复和筛选环节的计算开销。此外， 经典人工势场法（APF）虽然在实时性方面表现尚可，但当环境中出现“U 型”障碍物或狭窄通道等特殊结构时，合力场的固有特性很容易将无人机集群拖入局部极小值或导致死锁状态，使得算法无法继续推进，难以胜任复杂环境下对规划可靠性的要求。

近年来，生物启发式算法和自适应优化算法在连续优化问题中表现出较好的全局搜索能力，因此逐渐被引入复杂环境下的路径规划研究。这类算法通常通过模拟自然界中的生物行为，或通过参数自适应机制调整搜索过程，使种群在迭代中具备一定的跳出局部最优能力，为多障碍环境中的路径规划提供了新的思路。

螳螂搜索算法（MSA）模仿螳螂捕食过程中的追踪、伏击和攻击行为，将全局探索与局部开发结合起来，在部分多峰连续函数优化问题中取得了较好的结果。但将 MSA 直接用于二维障碍物环境下的路径规划时，仍会遇到一些问题。一方面，随机初始化难以保证初始解在复杂障碍空间中均匀分布，种群可能从一开始就集中在少数局部区域，影响后续搜索的多样性；另一方面，算法后期对搜索步长和方向的调节能力有限，容易过早收敛到局部较优路径，从而影响复杂环境下持续寻优的稳定性。

人工蜂鸟算法（AHA） 通过模拟蜂鸟的觅食行为和灵活飞行方式，利用访问表记录不同食源的访问情况，并结合轴向飞行、对角飞行和全向飞行三种方式，在引导觅食、领地觅食和迁徙觅食之间切换。这样的机制使 AHA 在连续优化问题中能够兼顾搜索范围和收敛速度。但在二维离散路径规划任务中，AHA 本身缺少针对航点连续性的专门约束。也就是说，算法可以更新航点坐标，却不能自然保证相邻航点之间的路径足够平滑。因此，生成的无人机航迹往往会出现较明显的折线和急转，难以满足实际飞行对路径可行性和飞行稳定性的要求。

基于成功历史参数自适应学习的差分进化（AL-SHADE） 算法利用成功历史参数自适应机制，根据搜索过程中成功个体的信息调整变异因子和交叉概率，并结合线性种群规模缩减策略，将计算资源逐步集中到较优个体附近。这一设计提高了差分进化算法处理高维连续优化问题的能力。但当 基于成功历史参数自适应学习的差分进化算法被直接用于二维离散航点规划时，算法虽然能够找到连接起点和终点的低代价路径，却没有显式约束相邻航点之间的几何连续性和转弯平滑度。因此，规划结果中仍可能出现急转、锯齿状波动等现象，影响路径的实际可执行性。
...
在无人机集群路径规划的研究中，编队控制是较早受到重视的基础问题。国外研究更偏向理论模型的构建，较早将多智能体一致性思想用于多个飞行器之间的协同控制。Ren 和 Beard 对分布式一致性控制及其在多飞行器系统中的应用进行了较为详细的阐述，这为后续编队控制研究提供了重要参考@RenBeard2008。相比之下，国内研究更多从实际任务的可行性出发，在虚拟领航者模型和一致性控制的基础上，结合无人机动力学限制、通信拓扑变化以及任务队形需求，讨论如何维持“人”字形或横队等典型编队@ZhangW2020UAVFormation。可以看出，国外研究主要关注通信拓扑和收敛性分析，国内研究则更重视队形能否适应具体航线和有限空间。随着任务环境从开阔空域逐渐转向城市低空、狭窄通道和复杂地形，固定队形的不足开始显现。为提高集群穿越受限环境的能力，队形约束调整、自适应重构、队形压缩和横向偏移等方法逐渐被引入相关研究@Zhang2019Formation。

在全局路径规划方面，国内外研究都大量采用 PSO、ACO、GA 等启发式算法。国外研究较早从算法本身出发，将粒子群优化、差分进化和新型生物启发式算法用于连续优化与路径搜索。Kennedy 和 Eberhart 提出的粒子群优化算法结构简单、收敛较快，因此较早被用于无人机路径搜索@Kennedy1995PSO；后续研究又引入螳螂搜索算法@MSA2023、L-SHADE@Tanabe2014LSHADE 以及 基于成功历史参数自适应学习的差分进化算法@ALSHADE2022 等方法，以增强复杂空间中的搜索能力。国内研究通常从无人机路径规划中的工程约束出发，对适应度函数进行改造，将路径长度、障碍物风险、平滑程度和约束惩罚等因素纳入评价过程，同时使用禁忌表、惩罚函数或精英保留机制提高可行解比例@Wang2021APF。二者的侧重点并不完全相同：国外更关注算法自身的搜索能力和参数自适应，国内则更关注路径是否可行、是否安全，以及是否便于仿真验证。
...
总体来看，国外研究在局部避障模型和控制理论方面积累较深，国内研究则更重视与全局路径和编队控制的耦合实现。但现有方法仍常把全局路径搜索、局部避障和编队控制分开设计，路径长度、轨迹平滑性、编队保持和机间安全裕度难以在同一优化过程中协调。基于此，本文选取基于成功历史参数自适应学习的差分进化算法作为主要全局路径规划框架，并通过外部档案相似度去重、障碍净空距离驱动的 Lévy-Gaussian 双态变异缩放因子和 B 样条曲线参数化方法进行改进。

== 研究内容

结合实际任务需求和前文分析的问题，本文以多障碍物环境下的无人机集群为研究对象，围绕编队控制、全局路径规划和局部动态避障展开研究。为使所提方法在复杂约束和密集障碍场景中具备可行性、稳定性与可复现性，本文将研究内容划分为以下四个方面：

1. 多无人机编队协同控制模型：基于虚拟领航者模型和多智能体一致性算法，构建多无人机协同控制框架，并说明邻接矩阵、拉普拉斯矩阵对通信拓扑和收敛过程的约束关系。针对二维平面内的编队保持与协同跟踪问题，设计同时考虑位置误差和速度误差的控制律，再结合速度、加速度和角速度等运动学约束进行物理可行性校验。在此基础上，引入横向偏置自适应调节机制，使集群在开阔区域和局部受限区域中都能收敛到人字型等预定队形，并在通道收缩和恢复过程中保留一定的队形弹性。
...
基于成功历史参数自适应学习的差分进化算法是在 L-SHADE 基础上发展而来的自适应差分进化算法。该算法保留成功历史参数记忆和线性种群规模缩减机制，并引入 current-to-Amean/1 变异策略，以利用外部档案中的优质历史信息。本文将基于成功历史参数自适应学习的差分进化算法作为主要全局路径规划框架。

基于成功历史参数自适应学习的差分进化算法的个体编码仍为二维航点向量：
...
*自适应策略选择。* 基于成功历史参数自适应学习的差分进化算法对两种变异策略采用概率调度。对每个个体生成随机数 $r in [0, 1]$，若 $r < P_s$，则执行 current-to-pbest/1；否则执行 current-to-Amean/1。第 $g$ 代结束后，根据两种策略产生改进解的成功比例更新 $P_s$：
...
在无人机路径规划中，基于成功历史参数自适应学习的差分进化算法的实现流程可以概括为：初始化种群、历史记忆和外部档案；为每个个体生成 $F_i$ 与 $(C R)_i$；按自适应概率选择变异策略；交叉得到试验个体；根据适应度执行选择并更新外部档案；最后更新历史记忆和种群规模。与 PSO、ACO、GA、MSA 和 AHA 相比，基于成功历史参数自适应学习的差分进化算法更适合连续航点坐标优化，能够在保持搜索多样性的同时逐步提高局部开发能力。但传统基于成功历史参数自适应学习的差分进化算法仍未显式考虑路径连续性、障碍威胁感知和外部档案相似解冗余问题，这也是后文进行改进的主要原因。
...
在算法层面，本章分别介绍了 PSO、ACO、GA、MSA、AHA 和 AL-SHADE 在二维航点优化问题中的设计方式。PSO 具有较快的收敛速度，ACO 和 GA 适合用于经典启发式对比，MSA 与 AHA 通过生物启发机制增强搜索多样性，而基于成功历史参数自适应学习的差分进化算法依靠历史参数记忆和线性种群缩减机制，更适合连续航点坐标的自适应优化。通过上述内容，可以明确各算法在全局路径规划中的基本作用、适用特点和潜在不足。下一章将在此基础上进一步建立无人机编队控制模型和局部避障模型，为全局路径的执行与验证提供支撑。
...
= 基于成功历史参数自适应学习的差分进化算法改进

== 基于成功历史参数自适应学习的差分进化算法的改进：TALG 框架

传统基于成功历史参数自适应学习的差分进化算法直接用于路径规划时，主要问题集中在档案利用、搜索步长和路径表达三个方面。首先，外部档案 $cal(A)$ 采用随机替换策略，迭代后期容易存入大量重复或高度相似的解，使档案多样性在不易察觉中下降。这样一来，current-to-Amean/1 变异策略所依赖的种群分布信息会变窄，档案均值的引导作用也会减弱。其次，变异因子 $F$ 采用柯西采样，对所有个体使用同一类参数生成方式，无法根据障碍物空间分布调整搜索尺度。开阔区域往往需要小步长精细搜索，而障碍物密集区域更需要较大步长帮助个体跳出危险通道，传统算法在这两种状态之间缺少自适应切换。除此之外，离散航点编码本身也不约束路径连续性。各航点坐标独立演化，即使相邻两代个体在搜索空间中较接近，解码后的路径仍可能出现明显折线和急转，进而使路径平滑性项 $S$ 与长度项 $L$ 的优化相互牵制。基于这些考虑，TALG（Threat-Adaptive Lévy-Gaussian）框架引入外部档案相似度去重、种群威胁度驱动的 Lévy-Gaussian 双态 $F$ 生成，以及 B 样条曲线参数化路径建模，分别改善档案多样性、环境感知搜索能力和路径几何连续性。

=== 外部档案相似度去重

传统基于成功历史参数自适应学习的差分进化算法在更新外部档案 $cal(A)$ 时，通常采用“档案满则随机替换一项”的方式存入被淘汰父代向量。这个方法实现起来很简单，但在路径规划任务中容易带来冗余。随着种群在迭代后期逐渐靠近某个较优解，很多被淘汰的父代向量会与该解非常接近，也就是在 $bb(R)^(2N)$ 空间中的距离很小。随机替换策略无法识别这类相似个体，档案中便可能积累大量近乎相同的解。这样计算得到的加权档案均值 $bold(x)_("mean")^t$ 会更像单一聚类中心的近似估计，难以反映多个独立优质区域的分布信息。
...
若父代与档案中某个成员在空间上很接近，但父代适应度更优，则用父代替换该相似成员。这样既减少重复解，也保留该区域内较好的搜索结果。若父代与档案成员相似且适应度不占优，则说明该区域已有更好的代表解，父代不再写入档案。若父代与档案中所有成员都保持足够距离，则说明它可能来自一个尚未覆盖的新区域；此时档案未满则直接追加，档案已满则随机替换一项，以保留传统 L-SHADE 档案更新中的随机性。
...
TALG 的第二项改进是引入种群威胁度（Population Threat）$T_i in [0, 1]$，用来描述单个候选路径面对障碍物时的风险水平。传统 AL-SHADE 在生成变异因子时基本不区分个体所处环境，而这里把路径与障碍物之间的空间关系压缩成一个标量。$T_i$ 越大，说明路径越靠近障碍物，甚至已经出现碰撞，此时算法应倾向于采用更大尺度的变异，让个体尽快离开危险区域。
...
/* *B 样条感知对齐。* 威胁度的计算基于 B 样条曲线采样点而非控制点，这一点至关重要。若直接对控制点计算净空距离，可能出现"控制点均安全、但控制点之间的 B 样条曲线段紧贴障碍物"的漏判情况——这正是传统方法在评估函数与优化变量之间存在表征鸿沟的体现。将 100 个 B 样条采样点投入广播距离计算虽然在单次威胁评估中引入了 $O(100 dot |cal(O)|)$ 的额外开销，但其带来的安全收益——避免优化器产出外观安全实则贴障的虚假可行路径——在障碍物密集场景中远超计算代价。
...
*传统方法的局限。* 传统 AL-SHADE 对所有个体都从柯西分布 $F_i ~ "Cauchy"(M_(F, h_i), 0.1)$ 中采样 $F$。柯西分布具有重尾特性，因此 $F$ 有小概率取到较大值（$F ≫ 1$），这在理论上可以帮助受困个体跳出局部最优。但它并不关心个体所处的环境风险。处在开阔安全区域的个体，以及已经贴近障碍物、接近碰撞的个体，在传统框架中获得大 $F$ 的概率完全一样。这样容易带来两个问题：安全个体可能被偶然的大步长扰动破坏已有收敛结果，而危险个体又不一定能及时获得足够强的逃逸步长。
...
适应度函数中的路径长度 $L$、平滑性 $S$、综合风险 $R$、通道宽度惩罚 $W$ 和编队保持代价 $F$ 都基于采样点集合 $cal(P)_i$ 计算，不再直接使用控制点折线。这样评价对象就是实际执行时更接近的光滑轨迹，而不是控制点连线形成的近似路径。
...
*降维效应。* B 样条参数化还可以在保留路径表达能力的同时降低搜索难度。在经典航点编码中，若设置 $N = 20$ 个中间航点，优化维度为 $D = 40$。如果改用少量控制点描述曲线主体形态，需要优化的自由控制点可减少到约 $5 ~ 7$ 个，对应搜索维度降至 $10 ~ 14$。在较低维度下，差分向量 $(bold(x)_(r_1) - bold(x)_(r_2))$ 中的有效信息更集中，交叉操作也更容易保留有意义的路径结构。因此，B 样条不仅改善路径平滑性，也降低了 基于成功历史参数自适应学习的差分进化算法在高维航点空间中的搜索压力。
...
本章围绕传统基于成功历史参数自适应学习的差分进化算法 在无人机路径规划中的不足，提出了 TALG 改进框架。传统基于成功历史参数自适应学习的差分进化算法 虽然具备较好的连续优化能力，但直接用于航点规划时仍存在三个比较明显的问题：外部档案容易积累相似解，变异参数对障碍物威胁不敏感，离散航点编码也容易生成折线式轨迹。针对这些问题，本章分别从档案维护、搜索步长和路径表达三个角度进行了改进。
...
传统 AL-SHADE 的终态轨迹最短，到达也最快，最终时间为 70.60 s。不过从图中可以看到，它的全局路径在若干障碍物之间选择较直接的穿越方式，僚机轨迹在局部区域需要依靠 APF 作横向修正，最终最小无人机间距为 18.88 m。AL-SHADE-TALG-QU 的最终时间为 73.00 s，略慢于传统 AL-SHADE，但最小无人机间距提升到 20.38 m，是图示结果中最高的安全裕度。同时，AL-SHADE-TALG-QU 的全局路径和领航者轨迹更平滑，僚机之间的相对间隔也更稳定，说明 B 样条参数化和威胁感知机制对轨迹连续性与编队安全性有一定改善。

总体来看，路径可视化结果与五项指标结果基本一致。传统 AL-SHADE 在任务效率上更占优势，而 AL-SHADE-TALG-QU 在路径较短、到达较快的基础上，提高了最小机间距和编队轨迹一致性，更适合安全裕度要求较高的无人机集群任务。
...
*稳步改进型（MSA、AHA、AL-SHADE）。* MSA、AHA 与传统 AL-SHADE 的收敛过程相对更平滑，前期下降后仍能在较长迭代区间内保持小幅改进。MSA 在约 50 代后逐渐稳定，最终适应度约为 6.31；AHA 曲线呈阶梯式下降，说明访问表机制和迁徙觅食策略能够周期性地引入新搜索方向，使算法在中后期仍保留一定探索能力；AL-SHADE 的下降过程较为连续，约在 60 代后接近稳定，外部档案与自适应参数记忆机制使其后期仍具有微弱改进能力。
...
#captab(
  ...
  | AL-SHADE (传统)      | 31             | 6.37               | 65                   | 连续下降-后期微调 |
...
从编队保持效果看，AL-SHADE-TALG-QU 的相对队形误差 RMS 为 7.989 m，是七种算法中最低的结果。与传统 AL-SHADE 的 9.214 m 相比，误差降低约 13.30%；与 PSO 的 9.977 m 相比，误差降低约 19.93%。这说明改进算法生成的路径对编队控制层更友好，能够减少局部急转和通道压缩对队形结构的扰动，使僚机在跟随领航者时保持更稳定的相对位置。

从安全性看，AL-SHADE-TALG-QU 的最小无人机间距达到 20.47 m，在所有算法中最大，说明其在飞行过程中保留的机间安全裕度最充分。相比之下，ACOR 和 MSA 的最小机间距分别为 18.06 m 和 18.59 m，安全裕度相对较弱。AL-SHADE 虽然路径较短、到达时间最优，但最小机间距为 19.12 m，低于改进算法。综合来看，AL-SHADE-TALG-QU 在路径长度和到达时间上接近传统 AL-SHADE，同时在平滑性、队形误差和机间安全距离三个关键指标上取得最优结果，综合飞行性能更均衡。
...
由表@exp-rank-table 可知，AL-SHADE-TALG-QU 的平均排名为 1.4，在七种算法中综合最优。其路径长度和到达时间均排名第二，与传统 AL-SHADE 的差距较小；同时在路径平滑性、队形误差和最小机间距三项指标上排名第一，优势主要体现在轨迹质量、编队稳定性和安全裕度方面。传统 AL-SHADE 的平均排名为 2.2，综合表现次优，特点是任务效率最高，但队形保持和机间安全不如 AL-SHADE-TALG-QU。MSA 的平均排名为 3.4，整体表现较均衡。GA 和 PSO 分别在最小机间距或收敛速度方面有一定优势，但综合排名落后于 MSA、AL-SHADE 和改进算法。ACOR 在本组指标中的平均排名最高，说明它在当前场景下不适合作为复杂障碍物环境中的首选规划算法。
...
（1）*效率优先场景*：如果任务主要关注路径长度和到达时间，可以优先考虑传统 AL-SHADE。其平均路径长度为 1540.46 m，到达时间为 71.30 s，均为七种算法中的最优结果，适合飞行时间敏感、环境安全裕度要求相对可控的任务。
...
全局路径规划部分先构建统一的适应度函数，把路径长度、路径平滑性、静态障碍风险、集群内部碰撞风险、通信风险、通道宽度约束和编队保持代价放入同一评价框架。在此基础上，本文对 PSO、ACOR、GA、MSA、AHA 和 AL-SHADE 等算法进行建模与对比。针对传统 AL-SHADE 在路径连续性、环境风险感知和外部档案多样性方面的不足，本文提出 AL-SHADE-TALG-QU 改进框架。该框架通过外部档案相似度去重减少冗余历史解，利用种群威胁度驱动的 Lévy-Gaussian 双态缩放因子增强障碍物密集区域的跳出能力，并借助 B 样条曲线参数化路径建模提升轨迹连续性和平滑性。
...
第二，从任务效率看，传统 AL-SHADE 在平均路径长度和领航者到达时间两项指标上表现最好，路径长度为 1540.46 m，到达时间为 71.30 s。AL-SHADE-TALG-QU 的路径长度为 1543.71 m，到达时间为 72.60 s，二者差距较小。这说明改进算法在提升轨迹质量和安全裕度的同时，基本保留了传统 AL-SHADE 的飞行效率。

第三，从轨迹平滑性和编队保持效果看，AL-SHADE-TALG-QU 取得最优结果。其平均航向偏转为 0.796°，低于传统 AL-SHADE 的 0.835°；相对队形误差 RMS 为 7.989 m，比传统 AL-SHADE 的 9.214 m 降低约 13.30%。这表明 B 样条参数化和威胁感知机制能够改善路径几何连续性，并减少僚机跟随过程中的队形扰动。
...
第五，综合五项指标排名，AL-SHADE-TALG-QU 的平均排名为 1.4，优于传统 AL-SHADE 的 2.2 和 MSA 的 3.4，综合飞行性能最好。该结果说明，改进算法虽然在单纯适应度收敛曲线上并非最优，但在路径平滑性、队形保持和安全裕度等实际飞行指标上更有优势，更适合复杂障碍物环境下的无人机集群协同路径规划任务。
