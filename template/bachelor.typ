...
近几年，国内关于无人机集群路径规划的研究进展较快。研究重点逐渐从单机路径搜索转向多机协同、编队保持、复杂障碍物避障和分层仿真验证。与国外偏重理论模型和通用算法框架的思路相比，国内工作更在意方法能否落到具体场景中，例如算法集成、仿真环境搭建，以及工程实现是否便于调试和复现。

*编队控制层研究。* 在编队控制方面，国内学者多以虚拟领航者模型和一致性控制理论为基础，把无人机运动学约束、通信拓扑变化和任务队形需求放在同一框架下，研究二维或三维空间中的队形保持问题。这类工作通常选取“V”型、人字型或横队作为典型编队形式，并通过位置误差、速度误差和邻居协同项约束僚机运动，使其围绕领航者保持期望相对位置@ZhangW2020UAVFormation。遇到狭窄通道或局部避障场景时，固定队形的适应性会变差，因此队形压缩、队形恢复和横向偏置调节等方法受到更多关注。这些处理使编队不必完全打散原有结构，也能穿过宽度受限区域。

*全局路径规划层研究。* 在全局路径规划方面，PSO、ACO、GA 等经典智能优化算法仍是国内研究中的常见选择。研究者一般会结合无人机路径规划特点对算法作出调整，例如构造同时考虑路径长度、障碍物风险、平滑度和约束惩罚的适应度函数，借助禁忌表或惩罚函数减少不可行路径，再通过坐标变异、精英保留和局部搜索改善收敛质量@Wang2021APF。随着新型群体智能算法的发展，MSA、AHA、AL-SHADE 等方法也开始用于无人机路径规划任务。不过，二维航点编码本身存在较明显的限制：路径容易形成折线，搜索维度偏高，轨迹连续性也不够理想。针对这一问题，B 样条、Bezier 曲线和分段多项式轨迹等参数化方法被用于降低优化维度，并改善规划路径的可飞行性@Liu2020UAVPath。

*局部避障与动态避碰研究。* 在局部避障方面，国内研究主要处理密集障碍物环境中的实时避碰和局部死锁问题。改进人工势场法在工程应用中仍较常见，具体做法包括重构斥力函数、引入切向旋转力、调整动态权重，以及把局部避障目标与全局引导点结合起来。在“U”型障碍物、狭窄通道或多障碍物密集分布场景中，单纯依靠径向斥力容易引发无人机振荡或停滞；加入切向绕行引导和速度约束校正后，局部避障成功率可以得到改善@Wang2021APF。对于多无人机集群而言，局部避障还需要和编队控制配合使用。若只关注单机避障，某架无人机的临时机动可能破坏整体队形，甚至使机间距离过近。

从现有工作看，国内无人机集群路径规划研究大多围绕工程应用和算法改进展开，但高密度障碍物环境中的问题仍比较突出。全局路径规划与局部避障之间衔接不够紧密，路径平滑性、编队保持和安全裕度也常被分开处理，统一评价与联合优化方法仍然不足。部分智能优化算法直接用于航点编码时，还容易生成折线轨迹或不可执行路径。面向复杂障碍物场景，仍有必要构建一种兼顾全局搜索、路径连续性、局部避障和编队稳定性的分层耦合规划框架。

=== 国内外研究比较与评价

从整体脉络看，国外研究在一致性控制理论、群体智能优化算法和实时避障模型方面积累较多，方法体系也比较完整；国内研究更重视工程实现、算法改进和多模块集成验证。两类研究都说明，无人机集群路径规划已经不只是寻找一条最短路径，而是逐渐变成多目标、多约束和分层耦合的综合优化问题。只是放到复杂障碍物场景中，现有方法仍有不少问题需要继续处理。

经典群体智能算法在全局搜索和局部开发之间往往难以平衡。PSO 收敛较快，但在障碍物密集区域容易过早聚集到局部较优通道，使路径安全裕度不足；ACO 和 GA 的随机搜索能力较强，可是在连续高维航点空间中，又容易出现收敛慢、路径曲折或不可行解比例偏高等问题。新型生物启发式算法虽然增强了全局探索能力，但参数调节通常没有和具体环境风险直接联系起来，难以根据障碍物分布和个体受威胁程度动态改变搜索步长。另一个问题是，不少路径规划方法仍把全局路径搜索、局部避障和编队控制分开设计，优化过程中很难同时兼顾路径长度、轨迹平滑性、编队保持和机间安全裕度。尤其在二维高密度障碍物环境中，离散航点编码生成的折线路径连续性较差，后续局部避障和编队控制往往需要较大幅度修正，进而影响集群飞行的稳定性。

相比之下，AL-SHADE 作为一种基于成功历史记忆的自适应差分进化算法，更适合连续空间寻优。它通过历史成功参数记忆、自适应缩放因子与交叉概率更新，以及线性种群规模缩减机制，使种群在搜索早期保持较好的多样性，并在后期逐步加强局部开发能力。因此，将 AL-SHADE 用于无人机路径规划中的连续航点坐标优化，具有较好的适配性。原生 AL-SHADE 直接用于无人机集群路径规划时也有不足：离散航点编码容易生成折线轨迹，路径平滑性不够；变异参数主要依赖历史成功经验更新，对障碍物威胁程度的感知不足；外部档案中还可能累积大量相似个体，使档案均值的引导方向退化，进而削弱搜索多样性。

基于上述考虑，本文选取 AL-SHADE 作为主要全局路径规划框架，并在统一适应度函数和分层控制框架下进行改进。具体做法包括：设计外部档案相似度去重机制，减少冗余历史解对档案均值的干扰，增强 current-to-Amean/1 变异策略的引导有效性；构建基于障碍物净空距离的种群威胁度模型，并将其反馈到 Lévy-Gaussian 双态变异缩放因子生成过程中，使算法能够根据个体所处环境风险自适应调整搜索步长；引入 B 样条曲线参数化方法，将离散航点序列转化为连续光滑轨迹，提高路径可执行性。通过这些改进，本文旨在改善 AL-SHADE 在二维高密度障碍物环境下的路径平滑性、编队保持能力和机间安全裕度。

== 研究内容

结合实际任务需求和前文分析的问题，本文以二维多障碍物环境下的无人机集群为研究对象，围绕编队控制、全局路径规划和局部动态避障展开研究。为使所提方法在复杂约束和密集障碍场景中具备可行性、稳定性与可复现性，本文将研究内容划分为以下四个方面：

1. 多无人机编队协同控制机理与算法设计：基于虚拟领航者模型和多智能体一致性算法，构建多无人机协同控制框架，并说明邻接矩阵、拉普拉斯矩阵对通信拓扑和收敛过程的约束关系。针对二维平面内的编队保持与协同跟踪问题，设计同时考虑位置误差和速度误差的控制律，再结合速度、加速度和角速度等运动学约束进行物理可行性校验。在此基础上，引入横向偏置自适应调节机制，使集群在开阔区域和局部受限区域中都能收敛到“V”型、人字型等预定队形，并在通道收缩和恢复过程中保留一定的队形弹性。
2. 多障碍空间下的全局路径智能优化与轨迹平滑：面向密集障碍物和多约束二维环境，复现并改进粒子群（PSO）、蚁群（ACO）、遗传（GA）等经典启发式算法，通过禁忌表、路径可行性惩罚和坐标变异机制提高搜索效率与可行解比例。针对早熟收敛和高维路径编码问题，提出引入混沌初始化与非线性步长衰减的改进螳螂搜索算法（MSA）。针对差分进化算法用于离散航点编码时轨迹连续性不足的问题，进一步采用 B 样条曲线参数化的 AL-SHADE 算法，对路径维度进行降维并建立连续轨迹模型，从而改善全局规划结果的平滑性与可飞行性。
3. 基于改进人工势场的局部动态避障策略：针对局部动态障碍和“U”型障碍容易引发局部极小值的问题，在传统人工势场法（APF）的引力、斥力模型基础上加入切向旋转势场力，引导无人机沿障碍边缘绕行。该局部避障策略与全局路径引导点和编队控制律相结合，使无人机在不明显偏离全局任务目标的情况下，能够在障碍边缘和狭窄通道中完成较稳定的局部机动与逃逸。
4. 多层算法集成与仿真验证体系：构建统一的二维多障碍物仿真环境，将编队控制层、全局规划层和局部避障层放在同一场景中进行分层耦合验证。通过设置不同障碍密度和典型障碍分布，采用路径长度、平滑度、规划成功率、收敛迭代次数和队形保持误差等指标进行横向对比，分析所提方法在安全性、稳定性和效率方面的综合表现。
